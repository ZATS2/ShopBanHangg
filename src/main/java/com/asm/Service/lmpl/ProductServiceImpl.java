...
//	}
	...
	@Override
	public Page<Product> searchProductByKY(String keywords, Pageable pageable) {
	    
	    return pRepo.findByNameContaining(keywords, pageable);
	}
	
	@Override
	public Page<Product> findProductBetweenPrices(Double min, Double max, Pageable pageable){
		session.set("minPrice", min);
		session.set("maxPrice", max);
		return pRepo.findByPriceBetween(min, max, pageable);
	}
